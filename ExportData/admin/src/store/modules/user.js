import { login, register, logout, getInfo, refreshToken } from '@/api/user'
...
const actions = {
  ...
  register({ commit }, userInfo) {
    ...
    return new Promise((resolve, reject) => {
      register({ username: username.trim(), password: password, confirmPassword: confirmPassword }).then(response => {
        const data = response
        commit('SET_TOKEN', data.token)
        commit('SET_TOKENEXPIRE', data.expire)
        // setToken(data.token)
