...
//GetUserID 获取用户ID
func (a *UserRepository) GetUserID(sel *string, where interface{}) int32 {
	var user models.User
	if err := a.Base.First(where, &user, *sel); err != nil {
...
//UpdateUser 更新用户
func (a *UserRepository) UpdateUser(user *models.User, role *models.UserRole) bool {
	//使用事务同时更新用户数据和角色数据
...
func (a *UserRepository) DeleteUser(id int32) bool {
	//采用事务同时删除用户和相应的用户角色
	var (
